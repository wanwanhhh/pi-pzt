...
相机可选：索雷博 CS165MU（Zelux）走官方 Python SDK，界面第三个页签提供实时预览、
...
上面三个都不依赖测试框架，直接跑。另有 8 个离线单测（不碰硬件，也不依赖测试框架，逐个直接跑）：
...
`test_trace.py` 位置曲线的环形缓冲（窗口两端闭合、按时间裁剪、增长有界）。`hwtest.py` 覆盖连接/点到点/中途停止/急停/释放；
...
列表里直接显示，拷到别处也问得出来。相机这一路的实测结论与踩坑见 `docs/thorlabs/设备认识账.xml`。
