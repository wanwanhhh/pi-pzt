...
- **到位**：PI 是控制器给的硬件信号（+ 一段固定延时）；**XMT 只等满固定的稳定延时，
  什么都不判**（用户 2026-09 定，不读回、不按步距复核）。两者都叫 on_target，
  保证级别不同，所以 `StageStatus.settle_source` 要写进每点元数据；
  采图前那道「半个步距」的相对校验做不做，由 `step_check` 声明（PI True / XMT False）。
...
SETTLE_SOFTWARE = "software"  # 后端按读数判出来的结论（XMT：采图那一刻读数与目标的比较，**只记录不拦截**）
...
class StageProto(Protocol):
    ...
    # 采图前要不要**按步距再复核一次偏差**（scanner 那道相对校验，阈值 = 半个步距）。
    # PI：要 —— 补的是「步距比设备到达容差还小时，绝对判据失去分辨力」的盲区。
    # XMT：**不要**（用户 2026-09 定）：等满稳定延时就直接采图，偏差一概不拦、不中止、不标无效。
    # 代价见 docs/xmt/设备认识账.xml E8 / E12：设点丢帧是静默的，只能事后从读数看出来。
    step_check: bool
