...
def stage_idle_reads(link: Link, n: int = 20, gap_s: float = 2.0) -> dict:
    ...
    - 极差 = 读数噪声底（取两批较小的那个）。**口径**：极差是**读回单位**，
      ×0.75 才是 µm（见 docs/xmt/设备认识账.xml E11）。它现在只解释一件事：界面「到位」
      列的容差够不够宽 —— 生产端既不判稳、也不做任何到位判据（见 xmt_stage.wait_on_target）。
    ...
    print("     生产端不做任何判据：移动后只等满界面上的稳定延时就直接采图。")
    print("     这条噪声底用来判断等待时长会不会太短（走完 + 整定的时间），")
    print("     以及小步距下命令值与实际落点能差多少（读数照旧每点入库）。")
